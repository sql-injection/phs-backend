...
def eclipse_fitting_methods(RRints):
    """
    Input        :
...
def correlation_coef(RRints):
    ...
    r_rr = np.mean((ax1 - mu_rr) * (ax2 - mu_rr)) / \
        (np.sqrt(np.mean((ax1 - mu_rr) ** 2) * np.mean((ax2 - mu_rr) ** 2)))
